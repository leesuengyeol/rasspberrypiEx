...
int main()
{
	...
	while (1)
	{
		...
		//printf("DIFF Time");
		if(!digitalRead(SW1)&&state)//sw1이 눌렸다면 ==lap time
		{
			printf("Lap Time %d : %ld.%ld\n",rapNum,diff_tv.tv_sec, diff_tv.tv_usec);
			state=0;
			rapNum++;
		}
		else if(digitalRead(SW1) &&!state)
		{
			state =1 ;
		}
		if(!digitalRead(SW2)) // sw2가 눌렸다면 ==time print  + program end 
		{
			printf("Time : %ld.%ld\n",diff_tv.tv_sec, diff_tv.tv_usec);
			break;
		}
		printf("end Time : %ld.%ld\r", diff_tv.tv_sec, diff_tv.tv_usec);
		fflush(stdout);
	}
	/*
